...
- Do not add private APIs, login cookies, scraped media, or third-party resolver services.
...
- Treat Discourse Onebox as a separate server-side fetch path. Verify `blocked_onebox_domains` before claiming the forum does not contact Xiaohongshu/RedNote.
