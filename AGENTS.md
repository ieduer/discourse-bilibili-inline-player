...
- X support accepts only one exact public post identity from `x.com`/`twitter.com` (`www`/`mobile` included): `/<handle>/status/<ID>`, `/<handle>/statuses/<ID>`, those forms' `/photo/<n>` and `/video/<n>` views, and `/i/status/<ID>` or `/i/web/status/<ID>`. It must use only X's official `platform.twitter.com/embed/Tweet.html` player, keep `dnt=true`, keep the frame lazy, `no-referrer`, and sandboxed with `allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox`, and keep the original post link. Never scrape X, call a private or paid API, resolve short links, or route posts through a mirror such as `fxtwitter`, `vxtwitter`, or `nitter`; `t.co` stays unhandled. Drop `?s=`/`?t=` share-tracking parameters instead of forwarding them. Read only `postMessage` events whose origin is the player origin and whose source is that exact frame, and act only on its ready, empty-result, and resize reports. An X link Discourse already turned into an onebox belongs to Discourse: leave it alone. Every failure, including an unreachable X, must fall back to the source card with the original link, never an empty frame.
...
- BDFZ post support accepts only one exact article slug below `https://bdfz.net/posts/`. It frames that public server-rendered page directly, defaults to expanded and width-aware automatic scaling, keeps the original link, and exposes an accessible collapse/expand control. Keep the iframe lazy, no-referrer, and sandboxed with scripts and popups (`allow-scripts allow-popups allow-popups-to-escape-sandbox`) to render interactive charts such as ECharts and KaTeX, but strictly without forms, top-level navigation, or same-origin privileges (origin remains null/opaque to isolate forum and user center session state); do not route BDFZ posts through `expand-reader`. Preserve the 70% readability floor and the administrator's 100%-scale fallback. The BDFZ source owns sandbox-compatible CSS delivery: its SRI stylesheet links must use anonymous CORS. Do not add `allow-same-origin` to the sandbox.
- rdfz.net student blog support accepts only one exact article identity,
  `https://<handle>.rdfz.net/p/<slug>`, decided by that platform's own handle and
  slug grammar because every blog is a new hostname: 3-30 characters with at
  least three letters, no leading, trailing, or doubled hyphen, never all digits,
  and a single lowercase ASCII/digit/hyphen/CJK slug of at most 60 code points.
  `rdfz.net` is a shared zone, so `www`, the `blog` control plane, `go`,
  `recite`, `s`, `api`, `admin`, and `console` are excluded by name, and blog
  homes, archives, tag pages, feeds, media objects, and the console are outside
  the accepted path. That platform sends `X-Frame-Options: DENY`,
  `frame-ancestors 'none'`, and no CORS header, so it must never be framed or
  fetched by the browser: it is an `expand-reader` provider and nothing else.
  Render the article region and the replies region the service returns, keep the
  local allowlist re-sanitization on both, and keep reply state honest (count,
  none, or the author's own closed notice). The reply box stays on the blog; it
  is a cross-origin form that cannot work here. Article images may resolve only
  to the article's own host, `blog.rdfz.net`, and `img.bdfz.net`. Every failure
  falls back to the source card with the original link.
